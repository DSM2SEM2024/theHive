...
class UsuarioController {
    ...
    public function create($data) {
        ...
        $this->user->setNome($data->nome)->setEmail($data->email)->setSenha($data->senha)->setTipo($data->tipo);
    ...
    public function update($id, $data) {
        if (!isset($id, $data->nome, $data->email, $data->senha, $data->tipo)) {
            http_response_code(400);
            echo json_encode(["error" => "Dados incompletos para atualização do usuário."]);
            return;
        }

        $this->user->setUsuarioId($id)->setNome($data->nome)->setEmail($data->email)->setSenha($data->senha)->setTipo($data->tipo);
